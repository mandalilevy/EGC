...
$sql="SELECT * FROM `student` WHERE CONCAT(`adno`, `name`, `contact`, `alcontact`, `dpt`, `course`, `jd`, `cd`, `lecturer`, `status`, `cst`, `cet`) LIKE '%".$valuetosearch."%' and  dpt='Business'"; 
$search_result= filtertable($sql);
}
else{

$sql="SELECT * FROM `student` where  dpt='Business' ORDER BY `jd`  DESC" ;
$search_result=filtertable($sql);
...
<li class="nav-item">
     ...
     <form class="form-inline my-2 my-lg-0" action="allbusiness.php" method="post">
 ...
 <ul class="navbar-nav  mt-lg-0 ml-auto">
         ...
         <li class="nav-item">
        <a style="color: white; font-family:Verdana; cursor: pointer; font-weight: bolder;" class="nav-link" href="business.php">   <i class="fa fa-arrow-left" aria-hidden="true" style="font-weight:bolder;">Back</i></a>
      </li>
